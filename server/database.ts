...
if (count === 0) {
  ...
  const members = [
    ...
    ['Marcus Williams', 'Support Manager',    'Support', 'marcus.williams@company.com'],
    ['Emily Rodriguez', 'Support Specialist', 'Support', 'emily.rodriguez@company.com'],
    ['James Park',      'Billing Analyst',    'Billing', 'james.park@company.com'],
    ['Olivia Thompson', 'Billing Lead',       'Billing', 'olivia.thompson@company.com'],
  ];
...
const { count: callerCount } = db.prepare('SELECT COUNT(*) as count FROM callers').get();
...
if (callerCount === 0) {
  ...
  for (const caller of callers) insertCaller.run(...caller);
...
const { count: queueCount } = db.prepare('SELECT COUNT(*) as count FROM call_queue').get();
...
if (queueCount === 0) {
  ...
  const mockCalls = [
    // Sales - 1 waiting
    ['ACC-1001', 'Diana Prince',  'Sales',   'Interested in upgrading my plan, want pricing info',     'waiting',     1, 'Alex Johnson',    ago(14), null],
    // Support - 1 waiting
    ['ACC-1004', 'Lois Lane',     'Support', 'Mobile app keeps crashing after the latest update',      'waiting',     4, 'Emily Rodriguez', ago(4),  null],
    // Billing - 1 waiting
    ['ACC-1003', 'Clark Kent',    'Billing', 'Invoice amount seems incorrect, need to review charges', 'waiting',     6, 'Olivia Thompson', ago(9),  null],
  ];
  
  // Insert each mock call into the database
  for (const call of mockCalls) insertCall.run(...call);
