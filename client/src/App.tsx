...
function App() {
  ...
  const [form, setForm]                 = useState({ accountNumber: '', department: '', issue: '' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const r = await fetch(`${API}/api/queue`, {  // Submit a POST request to the API to submit a new call to the queue
        method: 'POST',
    ...
    finally { setLoading(false) }  // Set submission loading state back to false after the submission is complete
  }
      ...
      {isAuthenticated && (
        <section id="center">
          ...
          <div className="dash-controls">
            ...
            <button className="refresh-btn" onClick={loadQueue} disabled={queueLoading}>
          ...
          <div className="queue-list">
            {visibleCalls.length === 0 ? (
              ...
              visibleCalls.map(call => (
                <div key={call.id} className={`queue-card queue-card--${call.status}`}>
                  <div className="qc-header">
                    <span className="qc-account">
                      {call.caller_name ?? call.account_number}
                      <span className="qc-acctnum"> · {call.account_number}</span>
                    </span>
                    <span className={`dept-badge dept-badge--${call.department.toLowerCase()}`}>
                      {call.department}
                    </span>
                  ...
                  <div className="qc-footer">
                    ...
                    {call.status === 'waiting' && call.position != null && (
                      <span className="qc-pos">
                        #{call.position} · waiting {waitedMinutes(call.created_at)}m
                      </span>
                    )}
            ...
            <div className="confirmation-card">
              ...
              <div className="confirm-stats">
                ...
                <div>
                  ...
                  <span className="stat-value">~{submitResult.estimated_wait} min</span>
                </div>
              </div>
